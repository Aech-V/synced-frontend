...
const FloatingInput = ({ name, type, label, formik }) => {
    ...
    return (
        <div style={{ marginBottom: '20px' }}> {/* Increased margin for breathing room */}
            <motion.div 
            ...
            <AnimatePresence>
                {isError && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ color: '#ef4444', fontSize: '0.8rem', marginTop: '6px', marginLeft: '8px', fontWeight: '500' }}>
                        {formik.errors[name]}
                    </motion.div>
                )}
            </AnimatePresence>
...
const PasswordStrength = ({ password }) => {
    ...
    return (
        <div style={{ marginTop: '-4px', marginBottom: '28px', padding: '0 4px' }}>
...
const Auth = ({ onLoginSuccess }) => {
    ...
    const formik = useFormik({
        ...
        validationSchema: yup.object({
            ...
            username: authView === 'signup' ? yup.string().min(3, 'Too short').required('Required') : yup.string()
        }),
        onSubmit: async (values) => {
            ...
            try {
                ...
                triggerHaptic('error');
                setServerError(error.response?.data?.message || 'Authentication failed. Please verify your credentials.');
            }
        },
    });
    ...
    return (
        <div style={{ position: 'relative', width: '100vw', height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            ...
            >
                ...
                <form onSubmit={formik.handleSubmit}>
                    ...
                    <motion.button 
                        ...
                        style={{ width: '100%', padding: '16px', borderRadius: '14px', backgroundColor: 'var(--text-primary)', color: 'var(--bg-primary)', border: 'none', fontSize: '1rem', fontWeight: 'bold', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', cursor: 'pointer', marginTop: '8px' }}
                    >
                ...
                <div style={{ marginTop: '24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <button style={{ width: '100%', padding: '16px', borderRadius: '14px', backgroundColor: 'rgba(255,255,255,0.05)', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px', cursor: 'pointer', fontWeight: '600', transition: 'background-color 0.2s' }} onMouseEnter={(e) => e.target.style.backgroundColor = 'rgba(255,255,255,0.08)'} onMouseLeave={(e) => e.target.style.backgroundColor = 'rgba(255,255,255,0.05)'}>
            ...
            <AnimatePresence>
                {authView === 'signup' && (
                    <motion.div 
                        className="solid-panel" // FIX: Changed from glass-panel to solid-panel
                        initial={{ y: '100vh' }}
                        animate={{ y: 0 }}
                        exit={{ y: '100vh' }}
                    ...
                    >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px', flexShrink: 0 }}>
                            ...
                            <button onClick={() => setAuthView('login')} style={{ background: 'var(--bg-surface-hover)', border: 'none', color: 'var(--text-primary)', width: '36px', height: '36px', borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background-color 0.2s' }}>✕</button>
                        </div>
